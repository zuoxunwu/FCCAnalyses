...
processList_test = {
    'p8_ee_Zbb_ecm91_EvtGen_Bs2TauTau':{'chunks':1, 'fraction':0.001},
}
...
outputDirEos   = "/eos/experiment/fcc/ee/analyses_storage/flavor/Bs2TauTau/flatNtuples/winter2023/analysis_stage1_semilep_basic/"
...
class RDFanalysis():
    ...
    def analysers(df):
        df2 = (
            df
               ...
               .Define("muon_iso01",       "FCCAnalyses::ZHfunctions::coneIsolation(0.001, 0.1)(muons, ReconstructedParticles)")
               .Define("muon_iso02",       "FCCAnalyses::ZHfunctions::coneIsolation(0.001, 0.2)(muons, ReconstructedParticles)")
               .Define("muon_iso03",       "FCCAnalyses::ZHfunctions::coneIsolation(0.001, 0.3)(muons, ReconstructedParticles)")
               ...
               .Define("electron_iso01",       "FCCAnalyses::ZHfunctions::coneIsolation(0.001, 0.1)(electrons, ReconstructedParticles)")
               .Define("electron_iso02",       "FCCAnalyses::ZHfunctions::coneIsolation(0.001, 0.2)(electrons, ReconstructedParticles)")
               .Define("electron_iso03",       "FCCAnalyses::ZHfunctions::coneIsolation(0.001, 0.3)(electrons, ReconstructedParticles)")
               ...
               .Define("AllReco_px",        "ReconstructedParticle::get_px(ReconstructedParticles)")
               .Define("AllReco_py",        "ReconstructedParticle::get_py(ReconstructedParticles)")
               .Define("AllReco_pz",        "ReconstructedParticle::get_pz(ReconstructedParticles)")
               .Define("AllReco_phi",       "ReconstructedParticle::get_phi(ReconstructedParticles)")
               .Define("AllReco_eta",       "ReconstructedParticle::get_eta(ReconstructedParticles)")
               .Define("AllReco_energy",    "ReconstructedParticle::get_e(ReconstructedParticles)")
               .Define("AllReco_mass",      "ReconstructedParticle::get_mass(ReconstructedParticles)")
               .Define("AllReco_charge",    "ReconstructedParticle::get_charge(ReconstructedParticles)")
               .Define("AllReco_isChargedPion", "AllReco_mass > 0.139 and AllReco_mass < 0.140")

               .Define("chargedpion_px",       "AllReco_px[AllReco_isChargedPion]")
               .Define("chargedpion_py",       "AllReco_py[AllReco_isChargedPion]")
               .Define("chargedpion_pz",       "AllReco_pz[AllReco_isChargedPion]")
               .Define("chargedpion_phi",      "AllReco_phi[AllReco_isChargedPion]")
               .Define("chargedpion_eta",      "AllReco_eta[AllReco_isChargedPion]")
               .Define("chargedpion_energy",   "AllReco_energy[AllReco_isChargedPion]")
               .Define("chargedpion_mass",     "AllReco_mass[AllReco_isChargedPion]")
               .Define("chargedpion_charge",   "AllReco_charge[AllReco_isChargedPion]")

               .Define("n_chargedpions",     "return int(chargedpion_energy.size())")
               ...
               .Define("muon_thrustangle",         'Algorithms::getAxisCosTheta(EVT_thrust, muon_px, muon_py, muon_pz)')
               .Define("electron_thrustangle",     'Algorithms::getAxisCosTheta(EVT_thrust, electron_px, electron_py, electron_pz)')
               .Define("chargedpion_thrustangle",  'Algorithms::getAxisCosTheta(EVT_thrust, chargedpion_px, chargedpion_py, chargedpion_pz)')
               .Define("photon_thrustangle",       'Algorithms::getAxisCosTheta(EVT_thrust, photon_px, photon_py, photon_pz)')
               ...
               .Define("Tau23PiCandidates_phi",    "ROOT::VecOps::RVec<float> result; for (auto & p: Tau23PiCandidates_p4) {result.push_back(p.Phi());} return result;")
               .Define("Tau23PiCandidates_eta",    "ROOT::VecOps::RVec<float> result; for (auto & p: Tau23PiCandidates_p4) {result.push_back(p.Eta());} return result;")
               .Define("Tau23PiCandidates_energy", "ROOT::VecOps::RVec<float> result; for (auto & p: Tau23PiCandidates_p4) {result.push_back(p.E  ());} return result;")
               ...
               .Define("EVT_ThrustEmin_Nmuon", "float(myUtils::get_Npos(muon_thrustangle))")
               .Define("EVT_ThrustEmax_Nmuon", "float(myUtils::get_Nneg(muon_thrustangle))")
               .Define("EVT_ThrustEmin_Nelectron", "float(myUtils::get_Npos(electron_thrustangle))")
               .Define("EVT_ThrustEmax_Nelectron", "float(myUtils::get_Nneg(electron_thrustangle))")
               .Define("EVT_ThrustEmin_Nchargedpion", "float(myUtils::get_Npos(chargedpion_thrustangle))")
               .Define("EVT_ThrustEmax_Nchargedpion", "float(myUtils::get_Nneg(chargedpion_thrustangle))")

               .Filter("EVT_ThrustEmin_Nmuon + EVT_ThrustEmin_Nelectron == 1 and EVT_ThrustEmin_Nchargedpion > 0")
               .Filter(SelectionCut)

               ##########################################
               ##       tau -> 3 pi constituent info   ##
               ##########################################
    ...
    def output():
        branchList = [
                ...
                "taupos_elenu", "taupos_munu", "tauneg_elenu", "tauneg_munu",

                "n_muons", "n_electrons", "n_photons", "n_chargedpions",
                "muon_px", "muon_py", "muon_pz", "muon_phi", "muon_eta", "muon_energy", "muon_mass", "muon_charge",
                "muon_iso01", "muon_iso02", "muon_iso03", "muon_thrustangle",
                "electron_px", "electron_py", "electron_pz", "electron_phi", "electron_eta", "electron_energy", "electron_mass", "electron_charge",
                "electron_iso01", "electron_iso02", "electron_iso03", "electron_thrustangle",
                "photon_px", "photon_py", "photon_pz", "photon_phi", "photon_eta", "photon_energy", "photon_mass",
                "photon_thrustangle",
                "chargedpion_px", "chargedpion_py", "chargedpion_pz", "chargedpion_phi", "chargedpion_eta", "chargedpion_energy", "chargedpion_mass", "chargedpion_charge",
                "chargedpion_thrustangle",
                ...
                "Tau23PiCandidates_phi", "Tau23PiCandidates_eta", "Tau23PiCandidates_energy",
